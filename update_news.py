...
def get_ai_summary(text):
    ...
    if not api_key:
        print("错误：未找到 KUAI_API_KEY，请检查 YAML 文件中的 env 配置。")
        return "（API配置错误）"
    
    prompt = f"你是一个AI专家。请将以下动态总结为一句话中文干货（必须翻译成中文）。要求：30字以内。内容：\n{text}"
...
def render_html(data):
    ...
    # 放弃正则，使用简单的字符串切分和拼接，彻底解决 MemoryError
    for c_id, c_html in cats.items():
        start_mark = f""
        end_mark = f""
        if start_mark in content and end_mark in content:
            parts = content.split(start_mark)
            tail = parts[1].split(end_mark)
            content = parts[0] + start_mark + c_html + end_mark + tail[1]
...
def commit_changes():
    ...
    os.system('git commit -m "Auto Update [skip ci]" || echo "No changes"')
    os.system('git push')
